    ...
    def execute_backward_optimizers_(  # noqa C901
        ...
        if optimizer in (OptimType.EXACT_ROWWISE_ADAGRAD, OptimType.EXACT_ADAGRAD):
            ...
            for t in range(T):
                ...
                if sparse_ema_options is None:
                ...
                elif sparse_ema_options.ema_mode in [0, 1]:
                    if rowwise and weight_decay_mode in (
                        WeightDecayMode.COUNTER,
                        WeightDecayMode.COWCLIP,
                    ):
                        ...
                        (m1,) = split_optimizer_states[t]
                    # Embedding shrinking
                    bs[t].weight.data.mul_(sparse_ema_options.swap_coef)
                elif sparse_ema_options.ema_mode in [2, 3]:
                    if rowwise and weight_decay_mode in (
                        WeightDecayMode.COUNTER,
                        WeightDecayMode.COWCLIP,
                    ):
                    ...
                    m2_ref = (
                        bs[t]
                        .weight.cpu()
                        .mul(1.0 - sparse_ema_options.ema_coef)
                        .mul(sparse_ema_options.swap_coef)
                    )
                    torch.testing.assert_close(
                        m2.float()
                        .cpu()
                ...
                if sparse_ema_options is not None:
                    if sparse_ema_options.ema_mode in [2, 3]:
                        expected_keys.update(["sparse_ema", "iter"])
                    else:
